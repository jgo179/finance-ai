...
export const generateAiReport = async ({ month }: GenerateAiReportSchema) => {
  ...
  // pegar as transaçoes do mes recebido
  ...
  // mandar as transaçoes para o ChatGpt e pedir para ele gerar um relatório com insights
  const content = `Gere um relatório com insights sobre as minhas finanças, com dicas e orientações de como melhorar minha vida financeira. As transações estão divididas por ponto e vírgula. A estrutura de cada um é {DATA}-{TIPO}-{VALOR}-{CATEGORIA}. São elas:
    ${transactions
      .map(
        (transaction) =>
